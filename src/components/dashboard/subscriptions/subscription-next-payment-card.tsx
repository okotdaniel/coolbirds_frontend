...
import { PaymentMethodSection } from '@/components/dashboard/subscriptions/payment-method-section';
...
export function SubscriptionNextPaymentCard({ subscription, transactions }: Props) {
  ...
  return (
    <Card className={'bg-background/50 backdrop-blur-[24px] border-border p-6 @container'}>
      ...
      <PaymentMethodSection transactions={transactions} updatePaymentMethodUrl={subscription?.managementUrls?.updatePaymentMethod} />
    </Card>
  );
}
